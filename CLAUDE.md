...
- **`src/accrual_pipeline/`** — Python 3.11+ FastAPI backend. Mirrors an SAP BTP CPI iFlow that reads FI/MM/CO data, sends it to Claude for anomaly detection via tool use, and routes the response. Deployed to Fly.io (`fly.toml` → app `accrualagent`).
...
- **Two sides, one pay period.** `fetchers/payroll.py` returns both the Workday `Get_Payroll_Results` SOAP response (`fetch_workday_payroll_results`) and the SAP FI lines PECI is supposed to have posted (`fetch_peci_fi_lines`). The live Workday path is a hand-rolled SOAP envelope + WS-Security UsernameToken; it's stubbed (`NotImplementedError`) until a real tenant is wired up. **The demo never leaves `MOCK_MODE=true`** — there's no Workday developer tenant.
- **Two fixtures, both keyed to one biweekly period.** `tests/fixtures/workday_payroll_results.json` (50 employees, `BIWEEKLY-US-CORP`, pay period 2026-04-27 → 2026-05-10) and `tests/fixtures/fi_payroll_lines.json` (308 OData lines simulating PECI's S/4 posting). Both were emitted by a one-shot Python generator with `random.seed(42)`; **seven specific mismatches are seeded** (missing OT, wrong CC, missing-in-FI, bonus to wrong GL, FICA off by $50, termination not prorated, duplicate PECI delivery) so the demo always has something for Claude to find.
...
The backend has **two parallel deploy targets** — keep both working when you change the FastAPI surface:

- **Fly.io** (`Dockerfile` + `fly.toml`, app `accrualagent`). `MOCK_MODE=true` is baked in; override via `fly secrets set MOCK_MODE=false ...`. SQLite file lives on the machine's local disk and is wiped on redeploy.
- **Vercel Functions** (`api/index.py` + `vercel.json` + root `requirements.txt`). `api/index.py` adds `src/` to `sys.path` and re-exports `accrual_pipeline.main.app`; `vercel.json` rewrites all paths to `/api/index` so the FastAPI app sees raw URLs. Root `requirements.txt` is what Vercel installs — keep it in sync with `pyproject.toml` runtime deps. `.vercelignore` excludes `.env` because pydantic-settings reads it at runtime and would silently shadow `vercel env add` values.

Frontend: Vercel, separate project from the backend, with `BACKEND_API_URL` set in project settings to wherever FastAPI is reachable.
